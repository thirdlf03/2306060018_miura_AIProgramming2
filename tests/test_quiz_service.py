...
class TestQuizService:
    ...
    @patch('services.quiz_service.repository.get_available_countries')
    @patch('services.quiz_service.repository.get_public_holidays')
    def test_generate_true_false_question_holiday_true(self, mock_repo_get_holidays, mock_repo_get_countries, sample_countries):
        """真偽問題生成テスト（祝日が正解の場合）"""
        # モックの設定
    ...
    @patch('services.quiz_service.repository.get_available_countries')
    @patch('services.quiz_service.repository.get_public_holidays')
    def test_generate_true_false_question_holiday_false(self, mock_repo_get_holidays, mock_repo_get_countries, sample_countries):
        """真偽問題生成テスト（非祝日が正解の場合）"""
        # モックの設定
        mock_repo_get_countries.return_value = sample_countries
    ...
    def test_generate_guess_question_insufficient_countries(self, mock_repo_get):
        ...
        # モックの設定
        mock_repo_get.return_value = [{"countryCode": "JP", "name": "Japan"}, {"countryCode": "US", "name": "United States"}]  # 2国のみ
    ...
    @patch('services.quiz_service.repository.get_available_countries')
    @patch('services.quiz_service.repository.get_public_holidays')
    def test_generate_guess_question_no_holidays(self, mock_repo_get_holidays, mock_repo_get_countries, sample_countries):
