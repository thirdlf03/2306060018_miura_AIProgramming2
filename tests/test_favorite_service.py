...
class TestFavoriteService:
    ...
    def test_load_favorites_repository_error(self, mock_repo_load):
        ...
        with pytest.raises(Exception, match="お気に入りの読み込みに失敗しました: Repository Error"):
            load_favorites()
    ...
    def test_save_favorites_repository_error(self, mock_repo_save):
        ...
        with pytest.raises(Exception, match="お気に入りの保存に失敗しました: Repository Error"):
            save_favorites([])

    def test_remove_selected_favorites(self, sample_holidays, sample_favorites_dataframe):
        """選択された祝日の削除テスト"""
    ...
    def test_update_favorites_from_search(self, sample_holidays, sample_search_dataframe):
        """検索結果からのお気に入り更新テスト"""
        # 既存のお気に入り（異なる国の祝日を含む）
        existing_favorites = [
            Holiday(
